//! Poller Invezgo spike: cek pertama Senin–Jumat 09:00:00–09:00:59,
//! lalu setiap `INTERVAL_INVEZGO_SPIKE_POLL_SECS` (.env, default 120) di jam kerja.
...
use crate::on_demand::{
    fetch_invezgo_price_spikes, is_scrape_hours_at, is_stockbit_poller_scrape_hours,
};
...
pub struct InvezgoSpikeSnapshot {
    pub success: bool,
    pub message: String,
    pub data: Vec<SpikeEmiten>,
}

impl Default for InvezgoSpikeSnapshot {
    fn default() -> Self {
        Self {
            success: true,
            message: format!(
                "Menunggu poller Invezgo spike (cek pertama 09:00, lalu {}s)",
                poll_interval_secs()
            ),
            data: Vec::new(),
        }
...
fn poll_interval_secs() -> u64 {
    std::env::var("INTERVAL_INVEZGO_SPIKE_POLL_SECS")
        .ok()
...
/// Cek pertama hari kerja 09:00:00–09:00:59; seterusnya `INTERVAL_INVEZGO_SPIKE_POLL_SECS` di jam kerja.
/// `false` = dibatalkan karena subscriber habis.
async fn wait_before_next_poll(
    poller: &InvezgoSpikePoller,
    ...
    if !first_done_today {
        if in_first_check_window(now) {
            println!(
                "Invezgo spike poller: cek pertama hari ini (09:00:{:02})",
                now.second()
            );
            return true;
        }
        if after_first_check_window(now) && is_stockbit_poller_scrape_hours() {
            println!(
                "Invezgo spike poller: cek pertama hari ini (terlewat 09:00, langsung cek)"
            );
            return true;
        }
        ...
        println!(
            "Invezgo spike poller: cek pertama Senin-Jumat 09:00:{:02} pada {} (tunggu {wait_secs}s)",
            jitter,
    ...
    if wake >= next_first {
        ...
        println!(
            "Invezgo spike poller: cek pertama berikutnya {} (tunggu {wait_first}s, bukan interval {wait_secs}s)",
    ...
    if !is_scrape_hours_at(wake) {
        ...
        println!(
            "Invezgo spike poller: di luar jam kerja — cek berikutnya {} (tunggu {wait_window}s)",
            resume.format("%Y-%m-%d %H:%M:%S")
        );
        return poller.sleep_while_subscribed(wait_window).await;
    }

    println!("Invezgo spike poller: cek berikutnya dalam {wait_secs}s");
    poller.sleep_while_subscribed(wait_secs).await
}

pub struct InvezgoSpikePoller {
    notify: watch::Sender<InvezgoSpikeSnapshot>,
    loop_started: Arc<Mutex<bool>>,
    subscriber_count: AtomicUsize,
    subs_notify: Notify,
}

impl InvezgoSpikePoller {
    pub fn new() -> Arc<Self> {
        let (notify, _) = watch::channel(InvezgoSpikeSnapshot::default());
        Arc::new(Self {
            notify,
    ...
    pub fn register_subscriber(&self) {
        ...
        if prev == 0 {
            println!("Invezgo spike poller: subscriber pertama — poller aktif");
        }
    ...
    pub fn unregister_subscriber(&self) {
        loop {
            ...
            {
                if n == 1 {
                    println!("Invezgo spike poller: 0 subscriber — poller idle (tidak hit Invezgo)");
                }
                self.subs_notify.notify_waiters();
                break;
            }
    ...
    async fn wait_until_has_subscriber(&self) {
        loop {
            ...
            println!("Invezgo spike poller: idle (0 subscriber) — menunggu subscribe");
            notified.await;
        }
    }
    ...
    pub fn latest(&self) -> InvezgoSpikeSnapshot {
        self.notify.borrow().clone()
    }

    pub fn subscribe(&self) -> watch::Receiver<InvezgoSpikeSnapshot> {
    ...
    pub async fn ensure_loop_running(self: &Arc<Self>, session: Arc<Session>) {
        ...
        let cached = crate::invezgo_spike_cache::today_details().await;
        if !cached.is_empty() {
            let n = cached.len();
            let _ = self.notify.send(InvezgoSpikeSnapshot {
                success: true,
        ...
        *started = true;
        println!(
            "Invezgo spike poller: siap (jalan hanya jika ≥1 subscriber; cek pertama 09:00, lalu {}s)",
            poll_interval_secs()
        );
    }

    async fn publish(&self, snap: InvezgoSpikeSnapshot) {
    ...
    async fn run_loop(self: Arc<Self>, session: Arc<Session>) {
        ...
        loop {
            ...
            if crate::yahoo_market_holiday::is_poller_market_holiday().await {
                ...
                println!(
                    "Invezgo spike poller: cek berikutnya {} (tunggu {wait}s)",
            ...
            println!(
                "Invezgo spike poller: tick ({} subscriber)",
                self.subscriber_count()
            );
            match fetch_invezgo_price_spikes(session.as_ref()).await {
                Ok(acc) => {
                ...
                Err(e) => {
                    eprintln!("Invezgo spike poller: {e}");
                    let acc = crate::invezgo_spike_cache::today_details().await;
                    self.publish(InvezgoSpikeSnapshot {
                        success: false,
                        message: e,
