...
/// `invezgood.portofolio` (emiten_name) → intraday-data / chart Invezgo.
pub async fn fetch_invezgo_price_spikes(
    session: &Session,
) -> Result<Vec<crate::yahoo_atr::SpikeEmiten>, String> {
    let emitens = list_portofolio_emiten_codes(session).await?;
...
async fn list_portofolio_emiten_codes(session: &Session) -> Result<Vec<String>, String> {
    use futures_util::stream::{self, StreamExt, TryStreamExt};
    use scylla::DeserializeRow;

    const TOKEN_SEGMENTS: usize = 16;
    const SCAN_CONCURRENCY: usize = 8;
    const SCAN_Q: &str = "SELECT emiten_name FROM invezgood.portofolio \
    ...
    let segment_rows: Vec<Vec<String>> = stream::iter(0..TOKEN_SEGMENTS)
        .map(|seg| {
            let start = token_segment_start(seg, TOKEN_SEGMENTS);
            let end = token_segment_end(seg, TOKEN_SEGMENTS);
            async move {
                let mut rows = session
                    .query_iter(SCAN_Q, (start, end))
                    .await
                    .map_err(|e| format!("SELECT portofolio emiten_name: {e}"))?
                    .rows_stream::<Row>()
                    .map_err(|e| format!("portofolio emiten_name stream: {e}"))?;

                let mut out = Vec::new();
                while let Some(row) = rows
                    .try_next()
                    .await
                    .map_err(|e| format!("portofolio emiten_name row: {e}"))?
                {
    ...
    let mut out: Vec<String> = segment_rows.into_iter().flatten().collect();
    out.sort();
    out.dedup();
    Ok(out)
}
