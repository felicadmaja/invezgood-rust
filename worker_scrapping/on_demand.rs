...
/// Scrape bandarmology harian untuk banyak emiten (satu sesi browser).
/// `jobs`: `(emiten_code, days)` — hanya tanggal yang perlu scrape/refresh.
/// Single-flight global untuk set jobs yang sama.
pub async fn scrape_bandarmology_harian_multi_from_stockbit(
    session: Arc<Session>,
    jobs: &[(String, Vec<NaiveDate>)],
) -> Result<usize, String> {
    if jobs.is_empty() {
        return Ok(0);
    }

    let mut normalized: Vec<(String, Vec<NaiveDate>)> = Vec::with_capacity(jobs.len());
    for (code, days) in jobs {
        let code = code.trim().to_ascii_uppercase();
        if code.is_empty() || days.is_empty() {
            continue;
        }
    ...
    let mut rx = {
        ...
        if let Some(existing) = map.get(&flight_key) {
            println!(
                "On-demand bandarmology_harian multi: sudah berjalan — menunggu hasil..."
            );
            existing.clone()
        } else {
            let (tx, rx) = watch::channel::<Option<Result<usize, String>>>(None);
            ...
            let session = Arc::clone(&session);
            let key_spawn = flight_key.clone();
            let jobs_spawn = normalized.clone();
            tokio::spawn(async move {
                let result = run_bandarmology_harian_multi(session, &jobs_spawn).await;
                match &result {
                    Ok(n) => println!(
                        "On-demand bandarmology_harian multi selesai: {n} hari di-upsert."
                    ),
                    Err(e) => eprintln!("On-demand bandarmology_harian multi GAGAL: {e}"),
                }
                let _ = tx.send(Some(result));
                inflight_bandar_harian_map().lock().await.remove(&key_spawn);
            });
    ...
    loop {
        ...
        if rx.changed().await.is_err() {
            return Err(
                "on-demand bandarmology_harian multi: channel ditutup sebelum ada hasil".into(),
            );
        }
    }
}

async fn run_bandarmology_harian_multi(
    session: Arc<Session>,
    jobs: &[(String, Vec<NaiveDate>)],
) -> Result<usize, String> {
    let email = std::env::var("STOCKBIT_EMAIL")
    ...
    let total_days: usize = jobs.iter().map(|(_, d)| d.len()).sum();
    println!(
        "On-demand bandarmology_harian multi: {} emiten, {} hari...",
        jobs.len(),
        total_days
    );
    ...
    let result = async {
        ...
        for (idx, (code, days)) in jobs.iter().enumerate() {
            println!(
                "On-demand bandarmology_harian multi [{}/{}]: {code} ({} hari)...",
                idx + 1,
                jobs.len(),
                days.len()
            );
