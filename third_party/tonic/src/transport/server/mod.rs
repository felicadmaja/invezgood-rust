...
pub struct Server<L = Identity> {
    ...
    /// Grace period after `max_connection_age` GOAWAY before force-close.
    max_connection_age_grace: Option<Duration>,
    /// Idle (no RPC) duration before graceful GOAWAY; timer resets on each request.
...
impl<L> Server<L> {
    ...
    /// Grace period after [`Server::max_connection_age`] GOAWAY before the socket is
    /// force-closed if the client has not finished draining.
    #[must_use]
    ...
    /// Maximum duration a connection may remain idle (no inbound RPC) before the server
    /// sends a graceful GOAWAY. The idle timer resets on each request.
    #[must_use]
    pub fn max_connection_idle(self, max_connection_idle: Duration) -> Self {
        Server {
            max_connection_idle: Some(max_connection_idle),
            ..self
        }
    }
        ...
        ResBody::Error: Into<crate::Error>,
    {
        ...
        let max_connection_idle = self.max_connection_idle;
        ...
        loop {
            tokio::select! {
                ...
                io = incoming.next() => {
                    ...
                    let hyper_svc = RpcActivityHyperService {
                        inner: hyper_svc,
                        activity_tx,
...
/// Hyper service wrapper: track in-flight RPC + pulse idle timer reset.
#[derive(Clone)]
struct RpcActivityHyperService<S> {
    inner: S,
...
impl<F: Future> Future for RpcActivityFuture<F> {
...
impl<S, B> HyperService<Request<Incoming>> for RpcActivityHyperService<S>
where
    ...
    B: http_body::Body + Send + 'static,
{
    ...
    type Error = S::Error;
    type Future = RpcActivityFuture<S::Future>;
...
enum ConnectionTimeoutAction {
    GracefulGoAway,
    ForceClose,
}

async fn connection_age_timeout(
    max_connection_age: Option<Duration>,
    max_connection_age_grace: Option<Duration>,
) -> ConnectionTimeoutAction {
    if max_connection_age.is_none() {
        future::pending::<()>().await;
        unreachable!("pending never completes");
    }
    sleep(max_connection_age.unwrap()).await;
    if let Some(grace) = max_connection_age_grace {
        sleep(grace).await;
        ConnectionTimeoutAction::ForceClose
    } else {
        ConnectionTimeoutAction::GracefulGoAway
    }
}
    ...
    tokio::spawn(async move {
        {
            ...
            let mut age_timeout = pin!(Fuse {
                inner: Some(connection_age_timeout(
                    max_connection_age,
                    max_connection_age_grace,
                )),
            });

            let mut idle_sleep = pin!(sleep_or_pending(max_connection_idle));
            ...
            loop {
                tokio::select! {
                    ...
                    action = &mut age_timeout, if age_timeout.inner.is_some() => {
                        match action {
                            ConnectionTimeoutAction::GracefulGoAway => {
                                debug!("max connection age reached, graceful GOAWAY");
                                conn.as_mut().graceful_shutdown();
                                age_timeout.set(Fuse { inner: None });
                            }
                            ConnectionTimeoutAction::ForceClose => {
                                debug!("max connection age grace elapsed, force close");
                                break;
                            }
                        }
                    },
                    _ = &mut idle_sleep, if max_connection_idle.is_some() && in_flight.load(Ordering::SeqCst) == 0 => {
                        debug!("max connection idle reached, graceful GOAWAY");
                        conn.as_mut().graceful_shutdown();
                        idle_sleep.set(sleep_or_pending(None));
                    },
                    _ = activity_rx.recv() => {
                        if max_connection_idle.is_some() {
                            idle_sleep.set(sleep_or_pending(max_connection_idle));
                        }
                    },
                    _ = &mut sig => {
                        conn.as_mut().graceful_shutdown();
