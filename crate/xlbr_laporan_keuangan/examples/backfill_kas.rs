//! Backfill kolom `kas` (dan metrik lain) dari zip lokal `src/downloaded_xbrl`.
//!
//! Proses per emiten/tahun/kuartal ascending (Q1→Q4) agar dekumulasi YTD benar.
//!
//! Usage:
//!   cargo run -p xlbr_laporan_keuangan --example backfill_kas
//!   cargo run -p xlbr_laporan_keuangan --example backfill_kas -- UNTR
...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    eprintln!("Selesai: {ok} sukses, {failed} gagal, total {}", ok + failed);
    Ok(())
}
